...
export async function resolveCollisions(prof: any[]){
    for (const record of prof){
        ...
        await prisma.file.update({where: {uuid: data.id},
...
export async function updateSkinCache(nickname: string, ignore_cache: boolean = false){
    ...
    const cache = await prisma.file.findFirst({where: {uuid: uuid}});
    ...
    const nicks = await prisma.file.findMany({where: {default_nick: fetched_skin_data?.name}});
    if (nicks.length > 1){
        await resolveCollisions(nicks);
    }

    if (cache.default_nick !== fetched_skin_data.name){
        await prisma.file.update({where: {uuid: fetched_skin_data.id}, 
                                 data: {default_nick: fetched_skin_data.name, 
    ...
    const updated_data = await prisma.file.upsert({where: {uuid: fetched_skin_data.id},
                            create: {
                                uuid: fetched_skin_data.id,
                                nickname: fetched_skin_data.name.toLowerCase(),
                                default_nick: fetched_skin_data.name,
                                expires: new Date().getTime() + parseInt(process.env.TTL),
...
export async function searchNicks({fragment, take, page}: SearchParams): Promise<Search | null>{
    ...
    const cache = await prisma.file.findMany({where: {nickname: {contains: fragment}, valid: true}, 
                                              orderBy: {default_nick: "asc"},
                                              take: take, skip: take * page});
    if (!cache){
        return null;
    }
    const count: number = await prisma.file.count({where: {nickname: {contains: fragment}, valid: true}});
